...
CONST_SERVER_ASCENDING = True          #True= Server 123. False = Server 321
#CONST_SERVER_ASCENDING = False          #True= Server 123. False = Server 321
CONST_SWITCHING_COST_FUNCTION = 'D'    #'G' = Gathering method, 'D' = Distributed method
#CONST_SWITCHING_COST_FUNCTION = 'G'    #'G' = Gathering method, 'D' = Distributed method
...
class subchain:
    ...
    def F(self):
        ...
        if self.placement_function == 'g':
            return self.logc
        elif self.placement_function == 'd':
            return math.exp(self.logc)
...
class cost_of_virtual_switching():
    ...
    def cost_of_virtual_switching_calculator(self,chain, bell, partition, vnf, \
        ...
        """
        if (CONST_SWITCHING_COST_FUNCTION == 'G'):
            """
            Placement_function = 'g'
            """        
            if (vnf_length == 'l0' ):
                a = subchain(phin = float(1), phip = 1000, ovs_type = 'kernel', \
                                        ...
                                        if ((cpu[0] == myChainRun) and \
                                            ...
                                            if (b_first_vnf == True) :   
                                                ...
                                                myVNFListTotalCPUinPartition )
                                                        if ((cv_at_server+myVNFListTotalCPUinPartition) <= float(m1(s[3]))  ):
                                                            ...
                                                            service_chain_placement_vnf_server = (myChainRun, myBellRun, \
                                                                    ...
                                                                    s[2], s[3], s[4], ('l'+str(myVNFLengthinPartition)), \
                                                                    ('t'+str(myVNFListTotalCPUinPartition)), \
                                                                    ('cv'+str(cv_at_server)))
                                                            myVNFServer_2nd = s
...
#                                            """
            ...
            myChainCostTuple = (myChainRun,) 
            myChainCostTuple = myChainCostTuple + ((str(min_cost_at_bell)),)
            myChainCost.append(tuple(myChainCostTuple))
#        """
...
if __name__ == '__main__':
    ...
    print('==GREEDY_LONGEST_SUBCHAIN==')
    if (CONST_SERVER_ASCENDING):
        print('SERVER_ASCENDING=TRUE  (123)')
    else:
        print('SERVER_DESCENDING=TRUE (321)')

    if (CONST_SWITCHING_COST_FUNCTION == 'G'):
        print('SWITCHING_COST_PLACEMENT_FUNCTION=GATHERING METHOD')
    else:
        print('SWITCHING_COST_PLACEMENT_FUNCTION=DISTRIBUTED METHOD')    
